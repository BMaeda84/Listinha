...
import { getList, completeList } from '../services/api';
import ShoppingList from '../components/ShoppingList';
import CameraScanner from '../components/CameraScanner';

export default function ListDetail({ listId, household, onBack }) {
  ...
  function handleItemFound() {
    // Recarrega a lista quando um item novo é encontrado
    reload();
  }
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Lista de itens */}
      <ShoppingList list={list} onUpdate={reload} />

      {/* FAB de câmera (só aparece se lista está aberta) */}
      {list?.status === 'open' && (
      ...
      {/* Seletor de cômodo antes de abrir câmera */}
      {roomPickerOpen && (
        <div style={styles.sheet} onClick={() => setRoomPickerOpen(false)}>
          <div style={styles.sheetCard} onClick={e => e.stopPropagation()}>
            ...
            <button style={styles.cancelBtn} onClick={() => setRoomPickerOpen(false)}>
              Cancelar
            </button>
          </div>
        </div>
...
const styles = {
  ...
  cancelBtn: {
    ...
    color: '#9ca3af', fontSize: 15, cursor: 'pointer', padding: 10,
  },
};
