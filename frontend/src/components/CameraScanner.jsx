...
import { scanBarcode, addScannedItem } from '../services/api';
...
export default function CameraScanner({ listId, roomId, roomName, householdId, onItemFound, onClose }) {
  ...
  useEffect(() => {
    ...
    const controls = reader.decodeFromVideoElement(videoEl, async (result, err) => {
      ...
      // Debounce: ignora mesmo barcode por 5 segundos
      if (code === lastBarcode && now - lastBarcodeTime < 5000) return;
      lastBarcode = code;
      lastBarcodeTime = now;

      try {
        const { found, product } = await scanBarcode(code, householdId);
        if (found) {
          ...
          onItemFound?.({ item: product });
        } else {
          showToast(`Código ${code} não encontrado no catálogo`, 'warn');
        }
      } catch (e) {
  ...
  }, [isActive, listId, roomId, householdId, videoRef, onItemFound]);
      ...
      <div style={styles.controls}>
        ...
        {!isActive ? (
          ...
          <div style={styles.activeControls}>
            <button style={styles.btnSecondary} onClick={resetSession}>
              🔄 Nova sessão
            </button>
